<x-layouts.app>
    ...
    <section class="py-4 sm:py-8 md:py-12 bg-white">
                ...
                <div class="space-y-4 sm:space-y-6">
                    ...
                    <div class="flex flex-wrap items-center gap-3 sm:gap-4">
                        @if($product->sale_price && $product->sale_price != $product->price)
                            <span class="text-2xl sm:text-3xl font-bold text-green-600">₦{{ number_format($product->price, 2) }} - ₦{{ number_format($product->sale_price, 2) }}</span>
                        @else
                            <span class="text-2xl sm:text-3xl font-bold text-green-600">₦{{ number_format($product->price, 2) }}</span>
                        @endif
                        @if($product->stock > 0)
                    ...
                    <div>
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-3">Pick buy option ({{ count($product->options) }} options available)</h3>
                        <div class="relative">
                            <select id="product-option" class="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white text-gray-900 appearance-none cursor-pointer">
                    ...
                    <!-- Product Details -->
                    <div>
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-3">Product Details</h3>
                    ...
                    <!-- Quantity Selector -->
                    <div>
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-3">Quantity</h3>
                        <div class="flex items-center space-x-2 sm:space-x-3">
                            <button onclick="decreaseQuantity()" class="w-10 h-10 sm:w-12 sm:h-12 bg-gray-100 hover:bg-gray-200 active:bg-gray-300 rounded-lg flex items-center justify-center transition-colors duration-150">
                                <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"></path>
                                </svg>
                            </button>
                            <input type="number" id="quantity" value="1" min="1" max="{{ $product->stock }}" class="w-16 sm:w-20 h-10 sm:h-12 text-center text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                            <button onclick="increaseQuantity()" class="w-10 h-10 sm:w-12 sm:h-12 bg-gray-100 hover:bg-gray-200 active:bg-gray-300 rounded-lg flex items-center justify-center transition-colors duration-150">
                                <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-2 gap-3 sm:gap-4">
                        @if($product->stock > 0)
                        <button id="add-to-cart-btn" onclick="addToCart()" class="w-full text-white text-sm sm:text-base font-semibold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-2" style="background-color: #f54a00;" onmouseover="this.style.backgroundColor='#d63e00'" onmouseout="this.style.backgroundColor='#f54a00'">
                        ...
                        @else
                        <button id="add-to-cart-btn" disabled class="w-full bg-gray-400 cursor-not-allowed text-white text-sm sm:text-base font-semibold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg flex items-center justify-center space-x-2">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                            </svg>
                            <span id="cart-btn-text">Out of Stock</span>
                        </button>
                        @endif
                        
                        <a href="{{ route('cart') }}" class="w-full text-white text-sm sm:text-base font-semibold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-2" style="background-color: #633322;" onmouseover="this.style.backgroundColor='#4a2618'" onmouseout="this.style.backgroundColor='#633322'">
                    ...
                    <div class="border-t border-gray-200 pt-4 sm:pt-6">
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-3">Share this product</h3>
                        <div class="flex items-center gap-2 sm:gap-3">
                            <!-- WhatsApp -->
                            <a href="https://wa.me/?text={{ urlencode($product->name . ' - ₦' . number_format($product->price, 2) . ' ' . url()->current()) }}" 
                               target="_blank" 
                               class="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-green-500 hover:bg-green-600 text-white rounded-full transition-colors duration-200"
                            ...
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" 
                               target="_blank" 
                               class="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-blue-600 hover:bg-blue-700 text-white rounded-full transition-colors duration-200"
                            ...
                            <a href="https://www.tiktok.com/share?url={{ urlencode(url()->current()) }}" 
                               target="_blank" 
                               class="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-black hover:bg-gray-800 text-white rounded-full transition-colors duration-200"
                            ...
                            <button onclick="copyProductLink()" 
                                    class="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 hover:from-yellow-500 hover:via-pink-600 hover:to-purple-700 text-white rounded-full transition-all duration-200"
    ...
    <section class="py-8 sm:py-12 md:py-16 bg-gray-50">
        <div class="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8">
            <h2 class="text-xl sm:text-2xl font-bold text-gray-900 mb-6 sm:mb-8">Related Products</h2>
    ...
    <!-- Product Option Selection Modal -->
    <div id="option-selection-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <!-- Background overlay -->
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-black bg-opacity-0 transition-opacity duration-300" aria-hidden="true" onclick="closeOptionModal()"></div>

            <!-- Modal panel -->
            <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all duration-300 opacity-0 translate-y-4 sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 sm:translate-y-0 sm:scale-95">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">
                                Please select a product option before adding this item to your cart. Choose from the available options above to continue.
                            </p>
                        </div>
                    </div>
                </div>
                <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                    ...
                    <button type="button" 
                            class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 sm:mt-0 sm:w-auto sm:text-sm transition-colors duration-200"
                            onclick="closeOptionModal()">
                        Cancel
                    </button>
                </div>
    ...
    <script>
        ...
        // Modal functions for product option selection
